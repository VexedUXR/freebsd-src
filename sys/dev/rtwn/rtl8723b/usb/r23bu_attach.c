...
r23bu_attach_private(struct rtwn_softc *sc)
{
	...
	rs->super.ac_usb_dma_size = 0xf; /* XXX correct value? */
...
static void
r23bu_postattach(struct rtwn_softc *sc)
{
	/* XXX TODO? */
}
...
r23bu_attach(struct rtwn_usb_softc *uc)
{
	...
	/* Common part. */
	sc->sc_flags			= RTWN_FLAG_EXT_HDR; /* XXX Other flags? */
	...
	sc->sc_vap_preattach		= rtwn_nop_softc_vap; /* XXX r12a sets capabilities here. */
	sc->sc_postattach		= r23bu_postattach;
...
#else
	/* XXX Test without ucode? */
	sc->sc_set_media_status		= r23bu_set_media_status;
#endif
	...
	sc->cam_entry_limit		= R12A_CAM_ENTRY_COUNT; /* XXX */
	...
	//sc->sc_debug |= RTWN_DEBUG_STATE; /* XXX for rtwn_set_multi, neither vendor driver nor linux do this... */
	//sc->sc_debug = UINT32_MAX;
	sc->sc_debug = RTWN_DEBUG_CALIB;
	//sc->sc_debug = RTWN_DEBUG_XMIT_DESC | RTWN_DEBUG_INTR;
	//sc->sc_debug = RTWN_DEBUG_STATE | RTWN_DEBUG_CALIB;
