...
r23b_parse_rom(struct rtwn_softc *sc, uint8_t *buf)
{
	...
	for (int i = 0; i < sc->ntxchains; i++) {
		...
		/* XXX
		 * Vendor driver always uses 5 for one of these?
		 */
		...
		/* XXX
		 * parse btcoex stuff here?
		 * https://github.com/lwfinger/rtl8723bu/blob/master/hal/rtl8723b_hal_init.c#L3350
		 */
